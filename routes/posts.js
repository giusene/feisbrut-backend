...
router.get('/posts', (req, res) => {
  ...
  router.get('/posts/:id', (req, res) => {
    readFile(data=>{
      ...
      data.map((post)=>{if(parseInt(post.id)==postId){result=post}})
      res.send(result);
  ...
  router.patch('/posts/:id', (req, res) => {
    ...
    readFile(data => {
      let result;
      const postId = req.params["id"];
      posts.map((post)=>{if(post.id===postId){result=post}})   
      
      
      let newReq = req.body
      let newObj = {...result,...newReq}   
         
      data.map((post)=> {if(post.id===postId){data[data.indexOf(post)]=newObj}})
  ...
  router.delete('/posts/:id', (req, res) => {
    ...
    readFile(data => {
      ...
      data.map((post)=> {if(post.id===postId){  data.splice([data.indexOf(post)])}})
